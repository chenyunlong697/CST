...
def plot_annotated_14D_EN(x, yu, yl, z, t, out_path: Path):
    """
    3 spans labeled INSIDE the regions with same font; more headroom so 'Max thickness' is never clipped.
    """
    if not HAS_MPL:
        print("[INFO] matplotlib unavailable; skip:", out_path.name); return
    ...
    # ---- figure ----
    fig = plt.figure(figsize=(14.4, 7))   # 更高一些，整体“向上延伸”
    ax  = plt.gca()
    ...
    ax.axvspan(0.003, 0.030, facecolor="#bcdcff", alpha=0.55,
               edgecolor="#2f5b9a", linewidth=1.2, zorder=0.10)
    # Pressure-recovery：浅蓝
    ax.axvspan(0.60, 0.90, facecolor="#cfe8ff", alpha=0.35,
               edgecolor="#6fa7d8", linewidth=0.8, zorder=0.12)
    # Reflex：带斜线 hatch，与 PR 清晰区分
    ax.axvspan(0.70, 0.95, facecolor="#e6f0ff", alpha=0.22,
               edgecolor="#385f99", linewidth=1.0, hatch="////", zorder=0.14)
    ...
    ax.set_aspect('equal', adjustable='box')
    ax.set_xlabel("x/c"); ax.set_ylabel("y/c")
    ax.set_title("CST fit with 14D feature annotations", pad=32)

    # 边距：右侧给 legend，底/顶给外侧标签
    fig.subplots_adjust(left=0.09, right=0.70, bottom=0.22, top=0.92)
    ...
    y_top_lab  = ymax_curve + 1.25 * y_range   # ↑ 比之前更大：防止 Max thickness 被顶住
    y_bot_lab  = ymin_curve - 0.70 * y_range
    ax.set_xlim(-0.02, 1.02)
    ax.set_ylim(y_bot_lab - 0.10*y_range, y_top_lab + 0.10*y_range)

    # helpers
    def vline(xv): ax.axvline(xv, ls=":", lw=0.9, zorder=1.2)
    def label_top(xv, text, xshift=0):
        ax.annotate(text, (xv, y_top_lab), xytext=(xshift, 10),
                    textcoords="offset points", rotation=90,
                    ha="center", va="bottom", zorder=2.6)
    def label_bot(xv, text, xshift=0):
        ax.annotate(text, (xv, y_bot_lab), xytext=(xshift, -10),
                    textcoords="offset points", rotation=90,
                    ha="center", va="top", zorder=2.6)

    # ========== D) 在区域内部放置“同一字体”的大标签 ==========
    trans_data_ax = mtransforms.blended_transform_factory(ax.transData, ax.transAxes)
    REGION_FS = 12  # 三个区域标签统一字号
    def region_label(x0, x1, text, y_frac=0.82, color="#1f3f78"):
        xm = 0.5*(x0 + x1)
        ax.text(xm, y_frac, text, transform=trans_data_ax,
                ha="center", va="center", fontsize=REGION_FS,
                fontweight="bold", color=color, zorder=3.2)
    ...
    singles.sort(key=lambda s: s[1])

    xshifts = []
    for i, (_, xv, _) in enumerate(singles):
        sgn = -1 if i % 2 == 0 else +1
        shift = sgn * HSHIFT_BASE
        if i > 0 and abs(xv - singles[i-1][1]) < EPS_X:
            shift += sgn * HSHIFT_ADD
        xshifts.append(shift)

    for (side, xv, text), xs in zip(singles, xshifts):
        vline(xv)
        (label_top if side=="top" else label_bot)(xv, text, xshift=xs)

    # 极值点高亮
    ax.scatter([x_t], [np.interp(x_t, x, z + 0.5*t)], s=24, zorder=3.1)
    ax.scatter([x_f], [np.interp(x_f, x, z)],         s=24, marker='s', zorder=3.1)

    # legend 右侧
    ax.legend(loc="center left", bbox_to_anchor=(1.005, 0.5),
              framealpha=0.95, fontsize=9)

    plt.savefig(out_path, dpi=180)   # 不用 tight，避免意外裁切
    plt.close()
